...
// BaseURL is the GitHub API root README fetches hit. A var, not a
// const, so tests can point it at a httptest server.
...
func Fetch(owner, repo, token string) Result {
	...
	return fetchFrom(url, owner, repo, token, cached)
}
